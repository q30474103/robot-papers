...
双击 `release\Robot Papers-Setup.exe`，安装向导会引导选择论文库根目录、安装位置，以及桌面/开始菜单/开机启动快捷方式。默认使用 Codex 定时任务模式，无需 API Key；只有主动切换到兼容 API 模式时才需要填写模型与 API Key。论文库根目录默认是 `D:\paper`，也可以改成其他位置；配置会保存到 `%APPDATA%\Robot Papers\config.json`。

安装包由 `构建-Robot-Papers-安装包.cmd` 生成，输出在 `release\Robot Papers-Setup.exe`。首次安装完成后会自动使用向导中选择的论文库路径。
