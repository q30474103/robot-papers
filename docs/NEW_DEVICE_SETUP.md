...
1. 安装 Robot Papers，并选择论文库根目录。Windows 默认是 `D:\paper`，Linux 默认是 `~/Robot-Papers`。
...
```text
请在这台设备上完成 Robot Papers 的本地联动配置。Windows 优先读取 `%APPDATA%\Robot Papers\config.json`，Linux 优先读取 `~/.config/Robot Papers/config.json` 中的 `paperRoot`；不要输出配置文件全文。如果配置不存在，Windows 使用 `D:\paper`，Linux 使用 `~/Robot-Papers`。不要使用 OpenAI API，也不要要求 API Key。
...
PDF 保存到 `{paperRoot}/原文/YYYY-MM-DD/ShortName.pdf`；中文 Obsidian 笔记保存到 `{paperRoot}/obsidian/YYYY-MM-DD/ShortName.md`，实际使用当前操作系统的路径分隔符。文件名只用官方简称，不加日期。每篇提取 2–4 张最重要的主图，按正文顺序插入并逐图解释。笔记必须分析研究问题、核心方法、实验、优点、局限、具体改进点、可证伪假设、最小实验、基线、指标和复现资源。更新 `{paperRoot}/obsidian/具身智能论文索引.md`，然后运行 Robot Papers 自带的安全同步脚本。
